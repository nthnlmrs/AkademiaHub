@section('title', 'Create Lab Class')
...
<x-app-layout>
    <div class="max-w-2xl">
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold mt-2">Create Lab Class</h2>
        </div>

        <div class="glass-card p-6">
            <form method="POST" action="{{ route('admin_lab.classes.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="form-input" required placeholder="A, B, C...">
                    </div>
                    <div>
                ...
                <!-- Class type is always LAB - shown as info -->
                <div class="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-3 flex items-center gap-3">
                    ...
                    <span class="text-sm text-cyan-300">Class type is automatically set to <strong>LAB</strong></span>
                </div>
                        ...
                        @foreach($lecturers as $lecturer)
                            <label>
                                <input type="checkbox" name="lecturers[]" value="{{ $lecturer->id }}" class="form-checkbox"
                                    {{ in_array($lecturer->id, old('lecturers', [])) ? 'checked' : '' }}>
                                <span>{{ $lecturer->name }}</span>
                                <span class="text-slate-500 text-xs ml-auto">{{ $lecturer->nim_nip }}</span>
                            </label>
                        @endforeach
                        @if($lecturers->isEmpty())
                            <p class="text-sm text-slate-500 p-2">No lecturers available.</p>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="select-list">
                        @foreach($teachingAssistants as $ta)
                            <label>
                                <input type="checkbox" name="teaching_assistants[]" value="{{ $ta->id }}" class="form-checkbox"
                                    {{ in_array($ta->id, old('teaching_assistants', [])) ? 'checked' : '' }}>
                                <span>{{ $ta->name }}</span>
                        ...
                        @if($teachingAssistants->isEmpty())
                            <p class="text-sm text-slate-500 p-2">No TAs available. Promote students first.</p>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="select-list" style="max-height: 300px">
                        ...
                        @endforeach
                        @if($students->isEmpty())
                            <p class="text-sm text-slate-500 p-2">No students available.</p>
                        @endif
                    </div>
                </div>

                <div class="flex gap-3 pt-2">
                    <button type="submit" class="btn-primary">Create Lab Class</button>
